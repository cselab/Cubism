...
template <typename TGrid, typename TLab> class MeshAdaptation {
...
protected:
          ...
          for (int i = 0; i < nx; i += 2) {
                ElementType average =
                    0.125 * (b(i, j, k) + b(i + 1, j, k) + b(i, j + 1, k) + b(i + 1, j + 1, k) +
                             b(i, j, k + 1) + b(i + 1, j, k + 1) + b(i, j + 1, k + 1) +
                             b(i + 1, j + 1, k + 1));
  ...
  void ValidStates()
  {
    ...
    {
      BlockInfo & info = I[j];
      if (info.state == Compress && (info.index[0] % 2 == 1 || info.index[1] % 2 == 1 || info.index[2] % 2 == 1))
      {
        info.state      = Leave;
        (m_refGrid->getBlockInfoAll(info.level, info.Z)).state = Leave;
      }
            ...
            for (int k = 0; k < nz; k += 2)
              for (int j = 0; j < ny; j += 2)
                for (int i = 0; i < nx; i += 2) {
                  ...
                  Kernel_1D(Planes[0][0], Planes[1][0], Planes[2][0], Planes[3][0], Planes[4][0],
                            Ref[0], Ref[1]);
                  Kernel_1D(Planes[0][1], Planes[1][1], Planes[2][1], Planes[3][1], Planes[4][1],
                            Ref[2], Ref[3]);
                  Kernel_1D(Planes[0][2], Planes[1][2], Planes[2][2], Planes[3][2], Planes[4][2],
                            Ref[4], Ref[5]);
                  Kernel_1D(Planes[0][3], Planes[1][3], Planes[2][3], Planes[3][3], Planes[4][3],
                            Ref[6], Ref[7]);
  ...
  virtual void WENOWavelets5(double cm2, double cm, double c, double cp, double cp2, double &left,double &right)
  {
