   ...
   MyRange &DetermineStencil(const Interface &f, bool CoarseVersion = false)
   {
      ...
      else
      {
         ...
         else
         {
            ...
            const int s[3] = {code[0] < 1 ? (code[0] < 0 ? ((stencil.sx - 1) / 2 + Cstencil.sx) : 0) : nX / 2, code[1] < 1 ? (code[1] < 0 ? ((stencil.sy - 1) / 2 + Cstencil.sy) : 0) : nY / 2, code[2] < 1 ? (code[2] < 0 ? ((stencil.sz - 1) / 2 + Cstencil.sz) : 0) : nZ / 2};

            const int e[3] = {code[0] < 1 ? (code[0] < 0 ? 0 : nX / 2) : nX / 2 + (stencil.ex) / 2 + Cstencil.ex - 1, code[1] < 1 ? (code[1] < 0 ? 0 : nY / 2) : nY / 2 + (stencil.ey) / 2 + Cstencil.ey - 1, code[2] < 1 ? (code[2] < 0 ? 0 : nZ / 2) : nZ / 2 + (stencil.ez) / 2 + Cstencil.ez - 1};

            const int base[3] = {(f.infos[1]->index[0] + code[0]) % 2, (f.infos[1]->index[1] + code[1]) % 2, (f.infos[1]->index[2] + code[2]) % 2};

            const int aux = 1 << f.infos[1]->level;
            int Cindex[3];
            for (int d = 0; d < 3; d++) Cindex[d] = (f.infos[1]->index[d] + code[d] + aux * blocksPerDim[d]) % (aux * blocksPerDim[d]);
...
class SynchronizerMPI_AMR
{
   ...
   struct UnpacksManagerStruct
   {
      ...
      UnpacksManagerStruct(MPI_Comm a_comm)
      {
         ...
         MPI_Aint offset;
         MPI_Get_address(&p.offset, &offset);
         array_of_displacements[0] = offset - base;
         MPI_Aint lx;
         MPI_Get_address(&p.lx, &lx);
         array_of_displacements[1] = lx - base;
         MPI_Aint ly;
         MPI_Get_address(&p.ly, &ly);
         array_of_displacements[2] = ly - base;
         MPI_Aint lz;
         MPI_Get_address(&p.lz, &lz);
         array_of_displacements[3] = lz - base;
         MPI_Aint srcxstart;
         MPI_Get_address(&p.srcxstart, &srcxstart);
         array_of_displacements[4] = srcxstart - base;
         MPI_Aint srcystart;
         MPI_Get_address(&p.srcystart, &srcystart);
         array_of_displacements[5] = srcystart - base;
         MPI_Aint srczstart;
         MPI_Get_address(&p.srczstart, &srczstart);
         array_of_displacements[6] = srczstart - base;
         MPI_Aint LX;
         MPI_Get_address(&p.LX, &LX);
         array_of_displacements[7] = LX - base;
         MPI_Aint LY;
         MPI_Get_address(&p.LY, &LY);
         array_of_displacements[8] = LY - base;
         MPI_Aint CoarseVersionOffset;
         MPI_Get_address(&p.CoarseVersionOffset, &CoarseVersionOffset);
         array_of_displacements[9] = CoarseVersionOffset - base;
         MPI_Aint CoarseVersionLX;
         MPI_Get_address(&p.CoarseVersionLX, &CoarseVersionLX);
         array_of_displacements[10] = CoarseVersionLX - base;
         MPI_Aint CoarseVersionLY;
         MPI_Get_address(&p.CoarseVersionLY, &CoarseVersionLY);
         array_of_displacements[11] = CoarseVersionLY - base;
         MPI_Aint CoarseVersionsrcxstart;
         MPI_Get_address(&p.CoarseVersionsrcxstart, &CoarseVersionsrcxstart);
         array_of_displacements[12] = CoarseVersionsrcxstart - base;
         MPI_Aint CoarseVersionsrcystart;
         MPI_Get_address(&p.CoarseVersionsrcystart, &CoarseVersionsrcystart);
         array_of_displacements[13] = CoarseVersionsrcystart - base;
         MPI_Aint CoarseVersionsrczstart;
         MPI_Get_address(&p.CoarseVersionsrczstart, &CoarseVersionsrczstart);
         array_of_displacements[14] = CoarseVersionsrczstart - base;
         MPI_Aint level;
         MPI_Get_address(&p.level, &level);
         array_of_displacements[15] = level - base;
         MPI_Aint Z;
         MPI_Get_address(&p.Z, &Z);
         array_of_displacements[16] = Z - base;
         MPI_Aint icode;
         MPI_Get_address(&p.icode, &icode);
         array_of_displacements[17] = icode - base;
         MPI_Aint IDreceiver;
         MPI_Get_address(&p.IDreceiver, &IDreceiver);
         array_of_displacements[18] = IDreceiver - base;
   ...
   struct DuplicatesManager
   {
      struct cube // could be more efficient, fix later
      {
         ...
         {
            static constexpr std::array<int, 3> faces_and_edges[18] = {{0, 1, 1}, {2, 1, 1}, {1, 0, 1}, {1, 2, 1}, {1, 1, 0}, {1, 1, 2},

                                                                       {0, 0, 1}, {0, 2, 1}, {2, 0, 1}, {2, 2, 1}, {1, 0, 0}, {1, 0, 2}, {1, 2, 0}, {1, 2, 2}, {0, 1, 0}, {0, 1, 2}, {2, 1, 0}, {2, 1, 2}};
      ...
      void RemoveDuplicates(int r, std::vector<Interface> &f, int &offset, int &total_size, int NC, std::vector<InterfaceInfo> &fInfo)
      {
         ...
         {
            C.__needed(remEl);
            for (size_t k = 0; k < remEl.size(); k++) fInfo[remEl[k]].ToBeKept = false;
         }
         /*------------->*/ Clock.finish(23);
         ...
         for (auto &i : C.keepEl())
         {
            ...
            for (size_t kk = 0; kk < (*i).removedIndices.size(); kk++)
            {
               ...
               Synch_ptr->UnpacksManager.manyUnpacks[r].push_back({info.offset, L[0], L[1], L[2], srcx, srcy, srcz, info.LX, info.LY, info.CoarseVersionOffset, info.CoarseVersionLX, info.CoarseVersionLY, Csrcx, Csrcy, Csrcz, f[remEl1].infos[0]->level, f[remEl1].infos[0]->Z, f[remEl1].icode[1], f[remEl1].infos[1]->blockID_2});
         ...
         for (int iy = s[1]; iy < e[1]; iy += yStep)
         {
            ...
            for (int ix = s[0]; ix < e[0]; ix += xStep)
            {
               ...
               for (int c = 0; c < NC; c++)
               {
                  ...
                  dst[pos] = 0.125 * ((*(src + gptfloats * ((XX) + ((YY) + (ZZ)*nY) * nX) + comp)) + (*(src + gptfloats * ((XX) + ((YY) + (ZZ + 1) * nY) * nX) + comp)) + (*(src + gptfloats * ((XX) + ((YY + 1) + (ZZ)*nY) * nX) + comp)) + (*(src + gptfloats * ((XX) + ((YY + 1) + (ZZ + 1) * nY) * nX) + comp)) + (*(src + gptfloats * ((XX + 1) + ((YY) + (ZZ)*nY) * nX) + comp)) + (*(src + gptfloats * ((XX + 1) + ((YY) + (ZZ + 1) * nY) * nX) + comp)) + (*(src + gptfloats * ((XX + 1) + ((YY + 1) + (ZZ)*nY) * nX) + comp)) + (*(src + gptfloats * ((XX + 1) + ((YY + 1) + (ZZ + 1) * nY) * nX) + comp)));
                  pos++;
               }
            }
         }
   ...
   void AverageDownAndFill2(Real *dst, const BlockInfo *const info, const int code[3], const int *const selcomponents, const int NC, const int gptfloats)
   {
      ...
      const int s[3] = {code[0] < 1 ? (code[0] < 0 ? sC[0] : 0) : nX / 2, code[1] < 1 ? (code[1] < 0 ? sC[1] : 0) : nY / 2, code[2] < 1 ? (code[2] < 0 ? sC[2] : 0) : nZ / 2};

      const int e[3] = {code[0] < 1 ? (code[0] < 0 ? 0 : nX / 2) : nX / 2 + eC[0] - 1, code[1] < 1 ? (code[1] < 0 ? 0 : nY / 2) : nY / 2 + eC[1] - 1, code[2] < 1 ? (code[2] < 0 ? 0 : nZ / 2) : nZ / 2 + eC[2] - 1};
   ...
   void DefineInterfaces()
   {
      ...
      if (define_neighbors)
         for (int i = 0; i < (int)myInfos_size; i++)
         {
            ...
            for (int icode = 0; icode < 27; icode++)
            {
               ...
               else if (infoNeiTree.CheckCoarser())
               {
                  ...
                  if (infoNeiCoarserrank != rank)
                  {
                     ...
                     BlockInfo &test = grid->getBlockInfoAll(infoNeiCoarser.level, infoNeiCoarser.Znei_(code2[0], code2[1], code2[2]));
               ...
               else if (infoNeiTree.CheckFiner())
               {
                  ...
                  for (int B = 0; B <= 3; B += Bstep) // loop over blocks that make up face/edge/corner (4/2/1 blocks)
                  {
                     ...
                     const int nFine1           = infoNei.Zchild[max(code[0], 0) + (B % 2) * max(0, 1 - abs(code[0]))][max(code[1], 0) + temp * max(0, 1 - abs(code[1]))][max(code[2], 0) + (B / 2) * max(0, 1 - abs(code[2]))];
            ...
            else
            {
               ...
               lengths.push_back(l);
               if (Coarsened)
                  for (size_t j = 0; j < ToBeChecked.size(); j += 2) send_interfaces[ToBeChecked[j]][ToBeChecked[j + 1]].CoarseStencil = UseCoarseStencil(send_interfaces[ToBeChecked[j]][ToBeChecked[j + 1]]);
            }
      ...
      for (size_t i = 0; i < interface_ranks_and_positions.size(); i++)
      {
         ...
         for (int r = 0; r < size; r++) DM.RemoveDuplicates(r, send_interfaces[r].v, offsets[r], send_buffer_size[r], stencil.selcomponents.size(), send_interfaces_infos[r]);
         /*------------->*/ Clock.finish(18);
      }
 ...
 public:
   ...
   SynchronizerMPI_AMR(StencilInfo a_stencil, StencilInfo a_Cstencil, const int a_levelMax, const int a_nx, const int a_ny, const int a_nz, const int a_bx, const int a_by, const int a_bz, TGrid *_grid) : stencil(a_stencil), Cstencil(a_Cstencil), levelMax(a_levelMax), UnpacksManager(_grid->getWorldComm()), SM(a_stencil, a_Cstencil, a_nx, a_ny, a_nz, a_bx, a_by, a_bz)
   {
...
#pragma omp for schedule(runtime)
            for (size_t j = 0; j < ToBeAveragedDown[r].size(); j += 2)
            {
               ...
               {
                  const int s[3] = {code0[0] < 1 ? (code0[0] < 0 ? stencil.sx : 0) : nX, code0[1] < 1 ? (code0[1] < 0 ? stencil.sy : 0) : nY, code0[2] < 1 ? (code0[2] < 0 ? stencil.sz : 0) : nZ};
                  const int e[3] = {code0[0] < 1 ? (code0[0] < 0 ? 0 : nX) : nX + stencil.ex - 1, code0[1] < 1 ? (code0[1] < 0 ? 0 : nY) : nY + stencil.ey - 1, code0[2] < 1 ? (code0[2] < 0 ? 0 : nZ) : nZ + stencil.ez - 1};
...
#pragma omp for schedule(runtime)
               for (int i = 0; i < N; i++)
               {
                  ...
                  pack(info.block, info.pack, gptfloats, &selcomponents.front(), NC, info.sx, info.sy, info.sz, info.ex, info.ey, info.ez, nX, nY);
               }
            }
         }
      }
   ...
   void fetch(const BlockInfo &info, const int gptfloats, const size_t Length[3], const size_t CLength[3], const size_t ElemsPerSlice[2], Real *cacheBlock, Real *coarseBlock)
   {
      ...
      for (size_t jj = 0; jj < UnpacksManager.sizes[id]; jj++)
      {
         ...
         const int s[3] = {code[0] < 1 ? (code[0] < 0 ? stencil.sx : 0) : nX, code[1] < 1 ? (code[1] < 0 ? stencil.sy : 0) : nY, code[2] < 1 ? (code[2] < 0 ? stencil.sz : 0) : nZ};
         const int e[3] = {code[0] < 1 ? (code[0] < 0 ? 0 : nX) : nX + stencil.ex - 1, code[1] < 1 ? (code[1] < 0 ? 0 : nY) : nY + stencil.ey - 1, code[2] < 1 ? (code[2] < 0 ? 0 : nZ) : nZ + stencil.ez - 1};
         ...
         if (other.level == info.level)
         {
            ...
            unpack_subregion<Real>(&recv_buffer[otherrank][unpack.offset], &dst[0], gptfloats, &stencil.selcomponents[0], stencil.selcomponents.size(), unpack.srcxstart, unpack.srcystart, unpack.srczstart, unpack.LX, unpack.LY, 0, 0, 0, unpack.lx, unpack.ly, unpack.lz, Length[0], Length[1], Length[2]);

            if (unpack.CoarseVersionOffset >= 0)
            {
               const int sC[3] = {(stencil.sx - 1) / 2 + Cstencil.sx, (stencil.sy - 1) / 2 + Cstencil.sy, (stencil.sz - 1) / 2 + Cstencil.sz};

               const int s1[3] = {code[0] < 1 ? (code[0] < 0 ? sC[0] : 0) : nX / 2, code[1] < 1 ? (code[1] < 0 ? sC[1] : 0) : nY / 2, code[2] < 1 ? (code[2] < 0 ? sC[2] : 0) : nZ / 2};
               ...
               unpack_subregion<Real>(&recv_buffer[otherrank][unpack.offset + unpack.CoarseVersionOffset], &dst1[0], gptfloats, &stencil.selcomponents[0], stencil.selcomponents.size(), unpack.CoarseVersionsrcxstart, unpack.CoarseVersionsrcystart, unpack.CoarseVersionsrczstart, unpack.CoarseVersionLX, unpack.CoarseVersionLY, 0, 0, 0, L[0], L[1], L[2], CLength[0], CLength[1], CLength[2]);
            }
         }
         else if (other.level < info.level)
         {
            const int sC[3] = {code[0] < 1 ? (code[0] < 0 ? ((stencil.sx - 1) / 2 + Cstencil.sx) : 0) : nX / 2, code[1] < 1 ? (code[1] < 0 ? ((stencil.sy - 1) / 2 + Cstencil.sy) : 0) : nY / 2, code[2] < 1 ? (code[2] < 0 ? ((stencil.sz - 1) / 2 + Cstencil.sz) : 0) : nZ / 2};
            ...
            unpack_subregion<Real>(&recv_buffer[otherrank][unpack.offset], &dst[0], gptfloats, &stencil.selcomponents[0], stencil.selcomponents.size(), unpack.srcxstart, unpack.srcystart, unpack.srczstart, unpack.LX, unpack.LY, 0, 0, 0, unpack.lx, unpack.ly, unpack.lz, CLength[0], CLength[1], CLength[2]);
         }
         else
         {
            int B;
...
template <typename Real, typename TGrid>
std::set<int> SynchronizerMPI_AMR<Real, TGrid>::Neighbors;
template <typename Real, typename TGrid>
std::vector<BlockInfo *> SynchronizerMPI_AMR<Real, TGrid>::halo_blocks;
template <typename Real, typename TGrid>
std::vector<BlockInfo *> SynchronizerMPI_AMR<Real, TGrid>::inner_blocks;
template <typename Real, typename TGrid>
std::vector<std::vector<std::pair<int, int>>> SynchronizerMPI_AMR<Real, TGrid>::interface_ranks_and_positions;
template <typename Real, typename TGrid>
std::vector<GrowingVector<Interface>> SynchronizerMPI_AMR<Real, TGrid>::send_interfaces;
template <typename Real, typename TGrid>
std::vector<size_t> SynchronizerMPI_AMR<Real, TGrid>::lengths;
