                ...
                else if (infoNei.TreePos == CheckCoarser)
                {
                    ...
                    if (infoNeiCoarser.myrank != rank)
                    {
                        ...
                        if (info.index[0]/2 == test.index[0] && info.index[1]/2 == test.index[1] && info.index[2]/2 == test.index[2])
                        {
                            ...
                            interface_ranks_and_positions.back().push_back({infoNeiCoarser.myrank, (int)send_interfaces[infoNeiCoarser.myrank].size()-1});
                        }
            ...
            else
            {
                ...
                if (Coarsened) 
                    for (size_t j = 0 ; j < ToBeChecked.size() ; j +=2 )
                        send_interfaces[ToBeChecked[j]][ToBeChecked[j+1]].CoarseStencil = UseCoarseStencil(send_interfaces[ToBeChecked[j]][ToBeChecked[j+1]]);
            }            
        }//i-loop
