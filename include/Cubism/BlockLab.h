...
public:
  ...
  template <int dim> int getActualSize() const {
    assert(dim >= 0 && dim < 3);
  ...
  // virtual void apply_bc_update(const BlockInfo& info, const Real dt=0, const Real a=0, const Real
  // b=0) { }
...
// 0.couple of checks
    ...
    *m_CoarsenedBlock = NAN;
    ...
    // 2. put the ghosts into the cache
    {
      ...
      for (size_t j = 0 ; j < info.my_neighbors.size() ; j ++)
      {
        ...
        int code[3] = {   - info.index[0] + infoNei.index[0],
                          - info.index[1] + infoNei.index[1],
        ...
        for (int j = 0; j < nY / 2; j++)
          for (int i = 0; i < nX / 2; i++) {
            ...
            coarseElement = AverageDown(
                ...
                m_cacheBlock->Read(ix, iy + 1, iz + 1), m_cacheBlock->Read(ix + 1, iy + 1, iz + 1));
          }
  ...
  void SameLevelExchange(const BlockInfo &info, const int *const code, const int *const s,
    ...
    // code[2],info.level)) return; if (!grid.avail(info.level, info.Znei_(code[0],code[1],code[2])))
    // return; BlockType& b = grid(info.index[0] + code[0], info.index[1] + code[1], info.index[2] +
    // code[2],info.level);
    ...
    for (int iz = s[2]; iz < e[2]; iz++) {
      ...
      if ((e[1] - s[1]) % 4 != 0) {
        ...
        for (int iy = s[1]; iy < e[1]; iy += 4) {
          char *ptrDest0 =
              (char *)&m_cacheBlock->LinAccess(my_izx + (iy + 0 - m_stencilStart[1]) * m_vSize0);
          char *ptrDest1 =
              (char *)&m_cacheBlock->LinAccess(my_izx + (iy + 1 - m_stencilStart[1]) * m_vSize0);
          char *ptrDest2 =
              (char *)&m_cacheBlock->LinAccess(my_izx + (iy + 2 - m_stencilStart[1]) * m_vSize0);
          char *ptrDest3 =
              (char *)&m_cacheBlock->LinAccess(my_izx + (iy + 3 - m_stencilStart[1]) * m_vSize0);
          const char *ptrSrc0 =
  ...
  void FineToCoarseExchange(const BlockInfo &info, const int *const code, const int *const s,
    ...
    /*
      ...
      An edge has two finer blocks, corresponding to B=0 and B=3. The block B=0 is the one closer to
      the origin (0,0,0). A face has four finer blocks. They are numbered as follows, depending on
      whether the face lies on the xy- , yz- or xz- plane
          ...
          for (int iy = s[1]; iy < e[1]; iy += yStep) {
            ElementType *ptrDest = (ElementType *)&m_cacheBlock->LinAccess(
                ...
                (abs(code[1]) * (iy - m_stencilStart[1]) +
                 (1 - abs(code[1])) * (iy / 2 - m_stencilStart[1] + aux * (e[1] - s[1]) / 2)) *
                    m_vSize0);
            ...
            // average down elements of block b to send to coarser neighbor
            for (int ee = 0;
                 ee < (abs(code[0]) * (e[0] - s[0]) + (1 - abs(code[0])) * ((e[0] - s[0]) / 2));
                 ee++) {
    ...
    for (int iz = s[2]; iz < e[2]; iz++) {
      ...
      for (int iy = s[1]; iy < e[1]; iy++) {
        ...
        for (int ee = 0; ee < e[0] - s[0]; ee++) {
          ptrDest1[ee] =
              AverageDown(*(ptrSrc_0 + 2 * ee), *(ptrSrc_1 + 2 * ee), *(ptrSrc_2 + 2 * ee),
                          *(ptrSrc_3 + 2 * ee), *(ptrSrc_0 + 2 * ee + 1), *(ptrSrc_1 + 2 * ee + 1),
                          *(ptrSrc_2 + 2 * ee + 1), *(ptrSrc_3 + 2 * ee + 1));
        }
      }
    }
  ...
  void CoarseFineInterpolation(const BlockInfo &info) {
    ...
    for (int icode = 0; icode < 27; icode++) {
      ...
      const int sC[3] = {code[0] < 1 ? (code[0] < 0 ? ((m_stencilStart[0] - 1) / 2) : 0) : nX / 2,
      ...
      // /*comment to silence warnings*/code[0]<1? (code[0]<0 ? 0:nX/2 ) : nX/2+(m_stencilEnd[0])/2
      // + 1 + 0*(m_InterpStencilEnd[0] -1),
      // /*comment to silence warnings*/code[1]<1? (code[1]<0 ? 0:nY/2 ) : nY/2+(m_stencilEnd[1])/2
      // + 1 + 0*(m_InterpStencilEnd[1] -1),
      // /*comment to silence warnings*/code[2]<1? (code[2]<0 ? 0:nZ/2 ) : nZ/2+(m_stencilEnd[2])/2
      ...
      for (int iz = s[2]; iz < e[2]; iz += 1) {
        ...
        // /*comment to silence warnings*/const int my_izx = (iz-m_stencilStart[2])*m_nElemsPerSlice
        // + my_ix;
              ...
              for (int j = 0; j < 3; j++)
                for (int k = 0; k < 3; k++)
                  Test[i][j][k] = &m_CoarsenedBlock->Access(
                      XX - 1 + i - offset[0], YY - 1 + j - offset[1], ZZ - 1 + k - offset[2]);
