...
class BlockLab
{
  ...
  public:
    ...
    void prepare(Grid<BlockType,allocator>& grid, const int stencil_start[3],
                                                  ...
                                                  const bool _istensorial,
                                                  const int Istencil_start[3],
      ...
      */
      ...
      for (int B = 0 ; B <= 3 ; B += Bstep) //loop over blocks that make up face/edge/corner (respectively 4,2 or 1 blocks)
      {
        ...
        BlockType * b_ptr = grid.avail1(2*info.index[0] + max(code[0],0) +code[0]  + (B%2)*max(0, 1 - abs(code[0]))  ,
                                        ...
                                        2*info.index[2] + max(code[2],0) +code[2]  + (B/2)*max(0, 1 - abs(code[2]))  ,info.level+1);
            ...
            const int my_izx = ( abs(code[2])*(iz-m_stencilStart[2]) + (1-abs(code[2]) )*(iz/2-m_stencilStart[2] + (B/2)*(e[2]-s[2])/2)  )*m_nElemsPerSlice + my_ix;
                ...
                if (  ((e[1]-s[1])/yStep) % 4 != 0 )
                {
                    for(int iy=s[1]; iy<e[1]; iy+=yStep)
                    {
                        ...
                        for (int ee=0; ee< ( abs(code[0])*(e[0]-s[0]) + (1-abs(code[0]))*((e[0]-s[0])/2) ); ee++)
                        {
                            ptrDest[ee] = AverageDown(* (ptrSrc_0 + 2*ee   ),* (ptrSrc_1 + 2*ee   ),* (ptrSrc_2 + 2*ee   ),* (ptrSrc_3 + 2*ee   ),
                                                      * (ptrSrc_0 + 2*ee +1),* (ptrSrc_1 + 2*ee +1),* (ptrSrc_2 + 2*ee +1),* (ptrSrc_3 + 2*ee +1));
                        }                                 
                    }
                }
                else
                {
                    ...
                    {
                        ElementType * ptrDest0 = (ElementType*)&m_cacheBlock->LinAccess(my_izx + ( abs(code[1])*(iy+0*yStep-m_stencilStart[1]) + (1-abs(code[1]) )*((iy+0*yStep)/2-m_stencilStart[1] + aux*(e[1]-s[1])/2)  )*m_vSize0);
                        ElementType * ptrDest1 = (ElementType*)&m_cacheBlock->LinAccess(my_izx + ( abs(code[1])*(iy+1*yStep-m_stencilStart[1]) + (1-abs(code[1]) )*((iy+1*yStep)/2-m_stencilStart[1] + aux*(e[1]-s[1])/2)  )*m_vSize0);
                        ElementType * ptrDest2 = (ElementType*)&m_cacheBlock->LinAccess(my_izx + ( abs(code[1])*(iy+2*yStep-m_stencilStart[1]) + (1-abs(code[1]) )*((iy+2*yStep)/2-m_stencilStart[1] + aux*(e[1]-s[1])/2)  )*m_vSize0);
                        ElementType * ptrDest3 = (ElementType*)&m_cacheBlock->LinAccess(my_izx + ( abs(code[1])*(iy+3*yStep-m_stencilStart[1]) + (1-abs(code[1]) )*((iy+3*yStep)/2-m_stencilStart[1] + aux*(e[1]-s[1])/2)  )*m_vSize0);
    ...
    void CoarseFineExchange(const BlockInfo & info, const int * const code)
    {
        ...
        const BlockInfo & infoNei = grid.getBlockInfoAll(info.level,info.Znei_(code[0],code[1],code[2]));                       
        BlockType * b_ptr = grid.avail1((infoNei.index[0])/2, (infoNei.index[1])/2, (infoNei.index[2])/2,info.level-1);
        if (b_ptr == nullptr) return;
        BlockType & b = * b_ptr;
