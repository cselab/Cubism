...
class GridMPI : public TGrid
{
 ...
 public:
   ...
   GridMPI(const int npeX, const int npeY, const int npeZ, const int nX, const int nY = 1, const int nZ = 1, const double _maxextent = 1, const int a_levelStart = 0, const int a_levelMax = 1, const MPI_Comm comm = MPI_COMM_WORLD, const bool a_xperiodic = true, const bool a_yperiodic = true, const bool a_zperiodic = true) : TGrid(nX, nY, nZ, _maxextent, a_levelStart, a_levelMax, false, a_xperiodic, a_yperiodic, a_zperiodic), timestamp(0), worldcomm(comm)
   {
   ...
   void UpdateBoundary()
   {
         ...
         for (int icode = 0; icode < 27; icode++)
         {
            ...
            else if (infoNeiTree.CheckFiner())
            {
                                                   ...
                                                   // (respectively 4,2 or 1 blocks)
               {
                  ...
                  int nFine1     = infoNei.Zchild[max(code[0], 0) + (B % 2) * max(0, 1 - abs(code[0]))][max(code[1], 0) + temp * max(0, 1 - abs(code[1]))][max(code[2], 0) + (B / 2) * max(0, 1 - abs(code[2]))];
      ...
      for (int r : Neighbors)
         if (r != rank)
         {
            ...
            if (send_buffer[r].size() != 0) MPI_Isend(&send_buffer[r][0], send_buffer[r].size(), MPI_INT, r, 123, worldcomm, &send_requests[send_requests.size() - 1]);
            else
            {
      ...
      for (int r : Neighbors)
         if (r != rank)
         {
            ...
            if (recv_size > 0)
            {
               ...
               MPI_Irecv(&recv_buffer[r][0], recv_buffer[r].size(), MPI_INT, r, 123, worldcomm, &recv_requests[recv_requests.size() - 1]);
            }
         }
   ...
   void UpdateBlockInfoAll_States(bool GlobalUpdate = false)
   {
      ...
      MPI_Allgatherv(myData, myLength, MPI_INT, All_data, AllLengths, displacement.data(), MPI_INT, worldcomm);
   ...
   SynchronizerMPIType *sync(const Processing &p)
   {
      ...
      {
         queryresult = new SynchronizerMPIType(p.stencil, Cstencil, TGrid::getlevelMax(), Block::sizeX, Block::sizeY, Block::sizeZ, blockperDim[0], blockperDim[1], blockperDim[2], this);
               ...
               for (int i2 = base[d2] - i_off[d2]; i2 <= base[d2] + i_off[d2 + 3]; i2++)
                  for (int i1 = base[d1] - i_off[d1]; i1 <= base[d1] + i_off[d1 + 3]; i1++)
                  {
                     ...
                     if (d == 0 || d == 3) n = TGrid::getZforward(I.level, i0, i1, i2);
                        ...
                        n = TGrid::getZforward(I.level, i2, i0, i1);
                     else /*if (d==2||d==5)*/
                        n = TGrid::getZforward(I.level, i1, i2, i0);

                     if (TGrid::Tree(I.level, n).rank() != myrank || TGrid::Tree(I.level, n).Exists() == false)
                     {
                        valid = false;
                        break;
                     }
                     ...
                     else if (added[TGrid::getBlockInfoAll(I.level, n).blockID - id_min] == true)
                     {
                        valid = false;
                     }
                  }
