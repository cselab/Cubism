...
class BlockLab
{
 ...
 public:
   ...
   static constexpr const int default_start [3] = {-1,-1,0};
   static constexpr const int default_end [3] = {2,2,1}; 
   void prepare(Grid<BlockType, allocator> &grid, const int stencil_start[3],
                const int stencil_end[3], const bool _istensorial, const int Istencil_start[3]=default_start,
      ...
      *m_CoarsenedBlock = NAN;
      ...
      // 2. put the ghosts into the cache
      {
         ...
         for (int icode = 0; icode < 27; icode++)
         {
            ...
            {
               //MadMike CoarseFineExchange(info, code);
            }
            ...
            //MadMike else if (infoNei.TreePos == CheckFiner)
            //MadMike    FineToCoarseExchange(info, code, s, e);
         } // icode = 0,...,26
         ...
         if (coarsened)
            ...
            {
               const int code[3] = {icode % 3 - 1, (icode / 3) % 3 - 1, (icode / 9) % 3 - 1};
               //MadMike FillCoarseVersion(info, code);
            }
#else
